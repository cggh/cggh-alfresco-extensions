 ...
 * 
 * @see{org.alfresco.repo.action.executer.MailActionExecuter}
 * @author Ian Wright
 */
    ...
    private void prepareAndSendEmail(final Action ruleAction, final NodeRef actionedUponNodeRef)
    {
        ...
        MimeMessagePreparator mailPreparer = new MimeMessagePreparator()
        {
            ...
            public void prepare(MimeMessage mimeMessage) throws MessagingException
            {
                ...
                //Customization to allow from address to be overridden by mail.parameters.from
                //iff it is the same as the default from address
                //otherwise mail.parameters.from is ignored when run as a script
                if (fromAddress.equals(from)) 
                {
                	
                } else if (fromActualUser != null && fromActualUser.length() != 0)
                {
