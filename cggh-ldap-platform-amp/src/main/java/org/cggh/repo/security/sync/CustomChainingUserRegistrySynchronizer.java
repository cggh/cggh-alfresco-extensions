        ...
        
{
    ...
    public SynchronizeDiagnostic testSynchronize(String authenticatorName)
    {
        ...
        if(instanceIds.contains(authenticatorName))
        {
            ...
            long groupLastModifiedMillis = getMostRecentUpdateTime(
                    CustomChainingUserRegistrySynchronizer.GROUP_LAST_MODIFIED_ATTRIBUTE, 
    ...
    private void synchronizeInternal(boolean forceUpdate, boolean isFullSync, final boolean splitTxns)
    {
        ...
        try
        {
            ...
            }, CustomChainingUserRegistrySynchronizer.LOCK_TTL / 2, CustomChainingUserRegistrySynchronizer.LOCK_TTL / 2,
                        ...
                        try
                        {
                            ...
                            if (mbeanServer != null && mbeanServer.isRegistered(name))
                            {
                                ...
                                for (MBeanAttributeInfo attribute : attributes)
                                {
                                    ...
                                    CustomChainingUserRegistrySynchronizer.logger.debug(attribute.getName() + " = " + value);
                                }
                            }
                        }
        ...
        {            
            ...
            if (lockToken != null)
            {
                ...
                do
                {
                    ...
                    try
                    {
                        lockRefresher.awaitTermination(CustomChainingUserRegistrySynchronizer.LOCK_TTL, TimeUnit.MILLISECONDS);
                    }
        ...
        if (userName != null && !userName.equals(AuthenticationUtil.getSystemUserName()))
        {
            if (this.syncWhenMissingPeopleLogIn)
            {
                ...
                catch (Exception e)
                {
                    ...
                    CustomChainingUserRegistrySynchronizer.logger.warn("User authenticated but failed to sync with user registry", e);
                }
            ...
            boolean isFullSync, boolean splitTxns, final Set<String> visitedZoneIds, final Set<String> allZoneIds)
    {
        ...
        long lastModifiedMillis = forceUpdate ? -1 : getMostRecentUpdateTime(
                CustomChainingUserRegistrySynchronizer.GROUP_LAST_MODIFIED_ATTRIBUTE, zoneId, splitTxns);
        ...
        if (CustomChainingUserRegistrySynchronizer.logger.isInfoEnabled())
        {
            ...
            {
                CustomChainingUserRegistrySynchronizer.logger.info("Retrieving all groups from user registry '" + zone + "'");
            }
            else
            {
                CustomChainingUserRegistrySynchronizer.logger.info("Retrieving groups changed since "
        ...
        class Analyzer extends BaseBatchProcessWorker<NodeDescription>
        {
            ...
            public void process(NodeDescription group) throws Throwable
            {
                ...
                String groupShortName = CustomChainingUserRegistrySynchronizer.this.authorityService.getShortName(groupName);
            ...
            private Set<String> getContainedAuthorities(String groupName)
            {
                ...
                for (String parent : CustomChainingUserRegistrySynchronizer.this.authorityService.getContainingAuthorities(
                        null, groupName, true))
                {
            ...
            private Set<String> cacheContainedAuthorities(String groupName)
            {
                ...
                children = CustomChainingUserRegistrySynchronizer.this.authorityService.getContainedAuthorities(null,
                        groupName, true);
            ...
            private synchronized void updateGroup(NodeDescription group, boolean existed)
            {
                ...
                {
                    groupDisplayName = CustomChainingUserRegistrySynchronizer.this.authorityService.getShortName(groupName);
                }
                        ...
                        else
                        {
                            ...
                            if (logger.isWarnEnabled())
                            {
                                CustomChainingUserRegistrySynchronizer.logger.warn("Not adding group '"
                                        + CustomChainingUserRegistrySynchronizer.this.authorityService.getShortName(group)
                                        + "' to group '"
                                        + CustomChainingUserRegistrySynchronizer.this.authorityService.getShortName(parent)
            ...
            {
                return CustomChainingUserRegistrySynchronizer.this.personService.getUserNamesAreCaseSensitive() ? new TreeSet<String>()
                        : new TreeSet<String>(String.CASE_INSENSITIVE_ORDER);
            }

            private Map<String, Set<String>> newPersonMap()
            {
                return CustomChainingUserRegistrySynchronizer.this.personService.getUserNamesAreCaseSensitive() ? new TreeMap<String, Set<String>>()
                ...
                while (i.hasNext())
                {
                    ...
                    if (!toRetain.contains(child))
                    {
                        if (CustomChainingUserRegistrySynchronizer.logger.isDebugEnabled())
                        {
                            ...
                            CustomChainingUserRegistrySynchronizer.logger.debug("Ignoring non-existent member '"
                                    + CustomChainingUserRegistrySynchronizer.this.authorityService.getShortName(child)
               ...
               if ((syncDelete || !groupsToCreate.isEmpty()) && (isFullSync || !this.groupParentAssocsToDelete.isEmpty()))
               {
                    ...
                    CustomChainingUserRegistrySynchronizer.this.transactionService.getRetryingTransactionHelper()
                            .doInTransaction(new RetryingTransactionCallback<Void>()
                            {
                                public Void execute() throws Throwable
                                {
                                    allZonePersons.addAll(CustomChainingUserRegistrySynchronizer.this.authorityService
                                            .getAllAuthoritiesInZone(zoneId, AuthorityType.USER));
                                    allZoneGroups.addAll(CustomChainingUserRegistrySynchronizer.this.authorityService
                        ...
                        groupCreator.process(new BaseBatchProcessWorker<Map.Entry<String, Set<String>>>()
                        {
                            ...
                            public void process(Map.Entry<String, Set<String>> entry) throws Throwable
                            {
                                ...
                                if (groupDisplayName != null)
                                {
                                    ...
                                    if (CustomChainingUserRegistrySynchronizer.logger.isDebugEnabled())
                                    {
                                        CustomChainingUserRegistrySynchronizer.logger.debug("Creating group '"
                                                + groupShortName + "'");
                                    }
                                    // create the group
            ...
            private void maintainAssociationDeletions(String authorityName)
            {
                ...
                if (parentsToDelete != null && !parentsToDelete.isEmpty())
                {
                    for (String parent : parentsToDelete)
                    {
                        ...
                        CustomChainingUserRegistrySynchronizer.this.authorityService.removeAuthority(parent, authorityName);
                    }
                }
                    ...
                    if (CustomChainingUserRegistrySynchronizer.logger.isDebugEnabled())
                    {
                        for (String groupName : parents)
                        {
                            CustomChainingUserRegistrySynchronizer.logger.debug("Adding '"
                                    ...
                                    + CustomChainingUserRegistrySynchronizer.this.authorityService.getShortName(groupName)
                                    + "'");
                        }
                    }
                    try
                    {
                        CustomChainingUserRegistrySynchronizer.this.authorityService.addAuthority(parents, authorityName);
                    }
        ...
        lastModifiedMillis = forceUpdate ? -1 : getMostRecentUpdateTime(
                CustomChainingUserRegistrySynchronizer.PERSON_LAST_MODIFIED_ATTRIBUTE, zoneId, splitTxns);
        lastModified = lastModifiedMillis == -1 ? null : new Date(lastModifiedMillis);
        if (CustomChainingUserRegistrySynchronizer.logger.isInfoEnabled())
        {
            if (lastModified == null)
            {
                CustomChainingUserRegistrySynchronizer.logger.info("Retrieving all users from user registry '" + zone + "'");
            }
            else
            {
                CustomChainingUserRegistrySynchronizer.logger.info("Retrieving users changed since "
                        + DateFormat.getDateTimeInstance().format(lastModified) + " from user registry '" + zone + "'");
            }
        }
        ...
        class PersonWorker extends BaseBatchProcessWorker<NodeDescription>
        {
            ...
            public void process(NodeDescription person) throws Throwable
            {
                ...
                if (zones == null)
                {
                    ...
                    CustomChainingUserRegistrySynchronizer.this.personService.createPerson(personProperties, zoneSet);
                    CustomChainingUserRegistrySynchronizer.this.avatarService.setAvatar(personName, person.getProperties(), getLatestTime());
                }
        ...
        if (latestTime != -1)
        {
            setMostRecentUpdateTime(CustomChainingUserRegistrySynchronizer.GROUP_LAST_MODIFIED_ATTRIBUTE, zoneId, latestTime,
                    splitTxns);
        }
            ...
            class AuthorityDeleter extends BaseBatchProcessWorker<String>
            {
                ...
                public void process(String authority) throws Throwable
                {
                    if (AuthorityType.getAuthorityType(authority) == AuthorityType.USER)
                    {
                        if (CustomChainingUserRegistrySynchronizer.logger.isDebugEnabled())
                        {
                            CustomChainingUserRegistrySynchronizer.logger.debug("Deleting user '" + authority + "'");
                        }
                    ...
                    else
                    {
                        if (CustomChainingUserRegistrySynchronizer.logger.isDebugEnabled())
                        {
                            CustomChainingUserRegistrySynchronizer.logger.debug("Deleting group '"
                                    + CustomChainingUserRegistrySynchronizer.this.authorityService.getShortName(authority)
                                    + "'");
                        }
        ...
        if (CustomChainingUserRegistrySynchronizer.logger.isInfoEnabled())
        {
            CustomChainingUserRegistrySynchronizer.logger.info("Finished synchronizing users and groups with user registry '"
                    + zone + "'");
    ...
    private long getMostRecentUpdateTime(final String label, final String zoneId, boolean splitTxns)
    {
        return this.transactionService.getRetryingTransactionHelper().doInTransaction(
                new RetryingTransactionCallback<Long>()
                {
                    ...
                    {
                        Long updateTime = (Long) CustomChainingUserRegistrySynchronizer.this.attributeService.getAttribute(
            ...
            boolean splitTxns)
    {
        this.transactionService.getRetryingTransactionHelper().doInTransaction(
                new RetryingTransactionHelper.RetryingTransactionCallback<Object>()
                {
                    public Object execute() throws Throwable
                    {
                        CustomChainingUserRegistrySynchronizer.this.attributeService.setAttribute(Long
                                .valueOf(lastModifiedMillis), CustomChainingUserRegistrySynchronizer.ROOT_ATTRIBUTE_PATH,
                                label, zoneId);
                        return null;
                    }
                }, false, splitTxns);
        ...
        if (this.syncOnStartup)
        {
            AuthenticationUtil.runAs(new RunAsWork<Object>()
            {
                public Object doWork() throws Exception
                {
                    ...
                    {
                        CustomChainingUserRegistrySynchronizer.logger.warn("Failed initial synchronize with user registries",
                                e);
                    }
                    return null;
                }
...
//        this.applicationEventPublisher.publishEvent(new SynchronizeDirectoryDeleteZoneEvent(this, zoneId, batchProcessNames));
    ...
    public Date getSyncStartTime()
    {
        Long start =  (Long)CustomChainingUserRegistrySynchronizer.this.attributeService.getAttribute(
                CustomChainingUserRegistrySynchronizer.ROOT_ATTRIBUTE_PATH, CustomChainingUserRegistrySynchronizer.START_TIME_ATTRIBUTE);
    ...
    {
        Long start =  (Long)CustomChainingUserRegistrySynchronizer.this.attributeService.getAttribute(
                CustomChainingUserRegistrySynchronizer.ROOT_ATTRIBUTE_PATH, CustomChainingUserRegistrySynchronizer.END_TIME_ATTRIBUTE);
    ...
    public String getLastErrorMessage()
    {
        String status =  (String)CustomChainingUserRegistrySynchronizer.this.attributeService.getAttribute(
                CustomChainingUserRegistrySynchronizer.ROOT_ATTRIBUTE_PATH, CustomChainingUserRegistrySynchronizer.LAST_ERROR_ATTRIBUTE);
        return status;
    }
    ...
    {
        String status =  (String)CustomChainingUserRegistrySynchronizer.this.attributeService.getAttribute(
                CustomChainingUserRegistrySynchronizer.ROOT_ATTRIBUTE_PATH, CustomChainingUserRegistrySynchronizer.SERVER_ATTRIBUTE);
        return status;
    }
    ...
    {
        String status =  (String)CustomChainingUserRegistrySynchronizer.this.attributeService.getAttribute(
                CustomChainingUserRegistrySynchronizer.ROOT_ATTRIBUTE_PATH, CustomChainingUserRegistrySynchronizer.STATUS_ATTRIBUTE);
        return status;
    ...
    {
        String status =  (String)CustomChainingUserRegistrySynchronizer.this.attributeService.getAttribute(
                CustomChainingUserRegistrySynchronizer.ROOT_ATTRIBUTE_PATH, CustomChainingUserRegistrySynchronizer.STATUS_ATTRIBUTE, zoneId);
        return status;
    }
    ...
    {
        String zoneId = AuthorityService.ZONE_AUTH_EXT_PREFIX + id;
        long time = getMostRecentUpdateTime(CustomChainingUserRegistrySynchronizer.PERSON_LAST_MODIFIED_ATTRIBUTE, zoneId, false);
    ...
    {
        String zoneId = AuthorityService.ZONE_AUTH_EXT_PREFIX + id;
        long time = getMostRecentUpdateTime(CustomChainingUserRegistrySynchronizer.GROUP_LAST_MODIFIED_ATTRIBUTE, zoneId, false);
    ...
    {
        String status =  (String)CustomChainingUserRegistrySynchronizer.this.attributeService.getAttribute(
                CustomChainingUserRegistrySynchronizer.ROOT_ATTRIBUTE_PATH, CustomChainingUserRegistrySynchronizer.LAST_ERROR_ATTRIBUTE, zoneId);
        return status;
    }
    ...
    {
        String status =  (String)CustomChainingUserRegistrySynchronizer.this.attributeService.getAttribute(
                CustomChainingUserRegistrySynchronizer.ROOT_ATTRIBUTE_PATH, CustomChainingUserRegistrySynchronizer.SUMMARY_ATTRIBUTE, zoneId);
        return status;
    }
